// Adapted from BambuStudio src/libslic3r/QuadricEdgeCollapse.cpp
// Changes: removed TBB dependency — parallel_for replaced with sequential loops.
...
Vec3d QuadricEdgeCollapse::calculate_vertex(double det, const SymMat &q) {
    double det_1 = -1 / det;
    double det_x = q.det(1, 2, 3, 4, 5, 6, 5, 7, 8);
    double det_y = q.det(0, 2, 3, 1, 5, 6, 2, 7, 8);
    double det_z = q.det(0, 1, 3, 1, 4, 6, 2, 5, 8);
...
QuadricEdgeCollapse::init(const indexed_triangle_set &its, ThrowOnCancel& throw_on_cancel, StatusFn& status_fn)
{
    ...
    VertexInfos   v_infos(its.vertices.size());
    {
        ...
        // calculate normals (sequential — TBB removed)
        for (size_t i = 0; i < its.indices.size(); ++i) {
            ...
            if (i % 1000000 == 0) {
                throw_on_cancel();
                status_fn(status_offset + (int)((i * status_normal_size) / its.indices.size()));
            }
        }
        ...
        for (size_t i = 0; i < its.indices.size(); i++) {
            ...
            if (i % 1000000 == 0) {
                throw_on_cancel();
                status_fn(status_offset + (int)((i * status_sum_quadric) / its.indices.size()));
            }
        }
    ...
    // calc error (sequential — TBB removed)
    ...
    for (size_t i = 0; i < its.indices.size(); ++i) {
        ...
        if (i % 1000000 == 0) {
            throw_on_cancel();
            status_fn(status_offset + (int)((i * status_calc_errors) / its.indices.size()));
        }
    }
    ...
    for (size_t i = 0; i < its.indices.size(); i++) {
        ...
        if (i % 1000000 == 0) {
            throw_on_cancel();
            status_fn(status_offset + (int)((i * status_create_refs) / its.indices.size()));
        }
    }
...
std::optional<uint32_t> QuadricEdgeCollapse::find_triangle_index1(uint32_t vi, const VertexInfo& v_info,
    uint32_t ti, const EdgeInfos& e_infos, const Indices& indices)
{
                                     ...
                                     const indexed_triangle_set &its)
{
    ...
    static const float dot_thr = 0.2f;
